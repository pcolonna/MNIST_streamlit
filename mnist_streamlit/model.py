...
def create(input_size):
    ...
    model.add(
        keras.layers.Dense(input_size, activation="relu", input_shape=(input_size,))
    )
...
def run_experiment(epochs, batch_size, progress_bar, status_text, val_acc_text, chart):
    ...
    model.fit(
        ...
        callbacks=[
            mc,
            myCallback(epochs, progress_bar, status_text, val_acc_text, chart),
        ],
    )
...
def predict(canvas, ground_truth_text, prediction_text, missing_model_text):
    ...
    try:
        if True:
            ...
            ground_truth_text.markdown(
                f"** Actual Number:** {np.argmax(random_mnist_example)}"
            )
...
def predict_from_drawing(img, prediction_text, missing_model_text_drawing):
    """Take the trained model, pick an example at random and returns its prediction."""

    try:
        if True:
            missing_model_text_drawing.text("")
            best_model = load_model("best_model.h5")
        ...
        missing_model_text_drawing.markdown(
            "** You need to click on Train Model first **"
        )
    except Exception as e:
        print(e)
...
class myCallback(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        best_val_accuracy = (
            ...
            else self.val_accuracy
        )
        self.val_acc_text.text(
            f"Best Validation Accuracy: {round(best_val_accuracy, 4)}"
        )
